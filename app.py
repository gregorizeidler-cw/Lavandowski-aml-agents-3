...
def analyze_user(user_data, betting_houses=None, pep_data=None, use_multiagent=True):
    ...
    gpt_analysis = get_gpt_analysis(prompt, use_multiagent=use_multiagent)
...
def run_bot():
    ...
    for i, user_data in enumerate(flagged_users):
        try:
            ...
            export_payload = analyze_user(user_data, betting_houses=betting_houses, pep_data=pep_data, use_multiagent=use_multiagent)
...
def main():
    with st.sidebar:
        ...
        use_multiagent = st.checkbox(
            "Usar Sistema Multi-Agente (Acusador, Defensor, Juiz)",
            value=True,
            help="Sistema de três agentes para reduzir falsos positivos"
        )
