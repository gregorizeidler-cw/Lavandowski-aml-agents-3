...
ACCUSER_SYSTEM_PROMPT = (
  "Você é um analista especialista em detecção de fraudes e lavagem de dinheiro, atuando como ACUSADOR. "
  "Seu papel é identificar e enfatizar padrões suspeitos que possam indicar atividades fraudulentas ou de lavagem de dinheiro. "
  "Você deve analisar os dados fornecidos com extremo rigor e destacar todos os sinais potenciais de comportamento suspeito. "
  "Como acusador, sua responsabilidade é proteger a instituição financeira de riscos reputacionais e legais. "
  "Procure por inconsistências, desvios de padrões normais, e quaisquer indícios que possam corresponder a técnicas conhecidas de lavagem de dinheiro. "
  "Você deve usar as alíneas da Carta Circular 4001 do BACEN para fundamentar suas acusações, citando especificamente quais se aplicam ao caso. "
  "Ao final de sua análise, forneça um score de risco de 1 a 10, justificando por que você acredita que este caso merece esta classificação. "
  "Seja vigilante e não deixe passar nenhum sinal potencial de fraude, mas baseie suas acusações em fatos concretos presentes nos dados."
)

DEFENDER_SYSTEM_PROMPT = (
  "Você é um analista especialista em compliance e análise contextual, atuando como DEFENSOR. "
  "Seu papel é analisar criticamente as acusações de fraude e lavagem de dinheiro, buscando explicações alternativas e legítimas para os padrões observados. "
  "Você deve considerar o contexto mais amplo das transações, o perfil do cliente, e as práticas comerciais normais no setor relevante. "
  "Como defensor, sua responsabilidade é evitar falsos positivos que prejudicariam clientes legítimos e a experiência do usuário. "
  "Procure por evidências que contradigam ou expliquem as suspeitas levantadas, como justificativas comerciais válidas, sazonalidade, ou outros fatores contextuais. "
  "Você não deve ignorar riscos reais, mas deve garantir que as acusações tenham base sólida antes de concordar com elas. "
  "Ao final de sua análise, forneça um score de risco de 1 a 10, justificando por que você acredita que este caso merece esta classificação. "
  "Seu objetivo é garantir uma análise justa e baseada em evidências, protegendo clientes legítimos de acusações injustificadas."
)

JUDGE_SYSTEM_PROMPT = (
  "Você é um analista sênior certificado pela ACAMS de Prevenção à Lavagem de Dinheiro e Financiamento ao Terrorismo, atuando como JUIZ. "
  "Seu papel é avaliar objetivamente as análises do acusador e do defensor, ponderando os argumentos de ambos os lados para chegar a uma conclusão final equilibrada. "
  "Você deve considerar a força das evidências apresentadas, o contexto mais amplo, o histórico do cliente, e as implicações regulatórias. "
  "Como juiz, sua responsabilidade é tomar a decisão final sobre se o caso deve ser considerado suspeito ou normal, e se requer validação adicional (BV). "
  "Você deve ser imparcial, reconhecendo tanto os sinais de risco quanto as explicações legítimas, e formando um julgamento baseado na preponderância das evidências. "
  "Ao final de sua análise, forneça um score de risco final de 1 a 10, explicando como você chegou a essa conclusão com base nas análises anteriores. "
  "Seu objetivo é alcançar o equilíbrio certo entre proteção contra riscos reais e evitar falsos positivos que prejudicariam clientes legítimos. "
  "Use as alíneas da Carta Circular 4001 do BACEN de forma criteriosa para fundamentar sua decisão final, citando apenas aquelas com evidência substancial. "
  "Ao final, sua conclusão deve ser clara: se o caso deve ser normalizado ou se requer validação adicional, especificando quais documentos devem ser solicitados se necessário."
)

def get_multiagent_analysis(prompt, models=["gpt-4o-2024-11-20"]*3):
  """
  Realiza a análise de um caso usando um sistema multi-agente com acusador, defensor e juiz.
  
  Args:
      prompt (str): Os dados do caso para análise.
      models (list): Lista de modelos a serem usados para cada agente [acusador, defensor, juiz].
      
  Returns:
      str: Uma string contendo a análise completa com as três perspectivas e a decisão final.
  """
  # Realiza a análise do acusador
  accuser_params = {
      "model": models[0],
      "messages": [
          {"role": "system", "content": ACCUSER_SYSTEM_PROMPT},
          {"role": "user", "content": f"Analise o seguinte caso como ACUSADOR, destacando todos os possíveis sinais de fraude ou lavagem de dinheiro:\n\n{prompt}"}
      ],
      "temperature": 0.0
  }
  
  try:
      accuser_response = client.chat.completions.create(**accuser_params)
      accuser_analysis = accuser_response.choices[0].message.content.strip()
  except Exception as e:
      accuser_analysis = f"Erro na análise do acusador: {str(e)}"
  
  # Realiza a análise do defensor, incluindo a análise do acusador
  defender_params = {
      "model": models[1],
      "messages": [
          {"role": "system", "content": DEFENDER_SYSTEM_PROMPT},
          {"role": "user", "content": f"Analise o seguinte caso como DEFENSOR, considerando a análise do ACUSADOR abaixo:\n\nCASO:\n{prompt}\n\nANÁLISE DO ACUSADOR:\n{accuser_analysis}"}
      ],
      "temperature": 0.0
  }
  
  try:
      defender_response = client.chat.completions.create(**defender_params)
      defender_analysis = defender_response.choices[0].message.content.strip()
  except Exception as e:
      defender_analysis = f"Erro na análise do defensor: {str(e)}"
  
  # Realiza a análise do juiz, incluindo as análises anteriores
  judge_params = {
      "model": models[2],
      "messages": [
          {"role": "system", "content": JUDGE_SYSTEM_PROMPT},
          {"role": "user", "content": f"Como JUIZ, avalie o seguinte caso considerando as análises do ACUSADOR e do DEFENSOR:\n\nCASO:\n{prompt}\n\nANÁLISE DO ACUSADOR:\n{accuser_analysis}\n\nANÁLISE DO DEFENSOR:\n{defender_analysis}"}
      ],
      "temperature": 0.0
  }
  
  try:
      judge_response = client.chat.completions.create(**judge_params)
      judge_analysis = judge_response.choices[0].message.content.strip()
  except Exception as e:
      judge_analysis = f"Erro na análise do juiz: {str(e)}"
  
  # Extrai o score de risco da análise do juiz
  risk_score = 0
  risk_score_match = re.search(r'Risco de Lavagem de Dinheiro: (\d+)/10', judge_analysis)
  if risk_score_match:
      risk_score = int(risk_score_match.group(1))
  
  # Determina a conclusão com base no score de risco
  if risk_score >= 4:
      conclusion = "suspicious"
  else:
      conclusion = "normal"
  
  # Combina todas as análises em um único relatório
  complete_analysis = (
      "## ANÁLISE COMPLETA DO SISTEMA MULTI-AGENTE\n\n"
      f"### ANÁLISE DO ACUSADOR\n{accuser_analysis}\n\n"
      f"### ANÁLISE DO DEFENSOR\n{defender_analysis}\n\n"
      f"### DECISÃO FINAL DO JUIZ\n{judge_analysis}\n\n"
  )
...
# Função modificada que serve como interface principal para análise
def get_gpt_multiagent_analysis(prompt, use_multiagent=True):
  """
  Interface principal para análise de casos, usando o sistema multi-agente ou o método tradicional.
  
  Args:
      prompt (str): Os dados do caso para análise.
      use_multiagent (bool): Se True, usa o sistema multi-agente; se False, usa o método tradicional.
      
  Returns:
      str: A análise completa do caso.
  """
  if use_multiagent:
      result = get_multiagent_analysis(prompt)
      return result["complete_analysis"]
  else:
      return get_chatgpt_response(prompt)

# Substituição da função original para manter a compatibilidade
