...
from gpt_utils import get_chatgpt_response, get_gpt_multiagent_analysis
import json
import decimal
import logging
import os
...
def get_gpt_analysis(prompt: str, use_multiagent=True) -> str:
  """Retorna a análise do GPT para o prompt fornecido."""
  if use_multiagent:
    return get_gpt_multiagent_analysis(prompt, use_multiagent=True)
  else:
...
def format_export_payload(user_id, description, business_validation):
  ...
  if has_error:
    ...
    risk_score = 0
    # Procura o score do juiz primeiro (que é o mais relevante no sistema multi-agente)
    judge_score_match = re.search(r'### DECISÃO FINAL DO JUIZ[\s\S]*?Risco de Lavagem de Dinheiro: (\d+)/10', clean_description)
    if judge_score_match:
      risk_score = int(judge_score_match.group(1))
    else:
      # Caso contrário, busca qualquer score de risco no texto
